...
function createPendingPasswordResetToken(payload) {
  return jwt.sign(
    { ...payload, type: "pending_password_reset" },
    process.env.JWT_SECRET || "change-this-secret",
    { expiresIn: "15m" },
  );
}

function createPasswordResetVerifiedToken(payload) {
  return jwt.sign(
...
async function findUserByEmail(role, email) {
  if (role === "admin") {
    const [admin] = await query(
      `SELECT admin_id AS id, admin_name AS name, admin_email AS email, admin_mobile AS phone_no
       FROM admin
       WHERE admin_email = ?`,
...
async function findAdminByPhone(phoneNumber) {
  const raw = String(phoneNumber || "").trim();
  const digits = raw.replace(/\D/g, "");
  const candidates = [normalizeIndianMobile(raw), raw];
  ...
  const admins = await query(
    `SELECT admin_id AS id, admin_name AS name, admin_email AS email, admin_mobile AS phone_no
     FROM admin
     WHERE admin_mobile IN (${uniqueCandidates.map(() => "?").join(", ")})`,
    uniqueCandidates,
  );
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!config) {
      return res.status(400).json({ message: "Email reset is not supported for this role" });
    }
    ...
    if (role === "admin") {
      const otp = String(Math.floor(10000 + Math.random() * 90000));
      const mailResult = await sendOtpEmail({ email: user.email, otp, role: "admin" });

      if (!mailResult.delivered) {
        return res.status(500).json({ message: mailResult.reason || "OTP email could not be delivered" });
      }

      return res.json({
        requiresResetOtp: true,
        resetToken: createPendingPasswordResetToken({
          role,
          userId: user.id,
          channel: "email",
          email: user.email,
...
router.post(
  "/forgot/phone/start/:role",
  asyncHandler(async (req, res) => {
    const role = req.params.role;
    const phoneNumber = String(req.body?.mobile || "").trim();

    if (!["farmer", "customer", "admin"].includes(role)) {
      return res.status(400).json({ message: "Mobile reset is not supported for this role" });
    }
    ...
    const user = role === "admin" ? await findAdminByPhone(phoneNumber) : await findUserByPhone(role, phoneNumber);
    if (!user) {
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    let payload;
    try {
      payload = jwt.verify(resetToken, process.env.JWT_SECRET || "change-this-secret");
    } catch {
      return res.status(401).json({ message: "Password reset session expired. Start again." });
    }

    if (payload.type !== "pending_password_reset") {
      return res.status(400).json({ message: "Invalid password reset session" });
    }

    const config = roleConfig[payload.role];
    if (!config) {
      return res.status(400).json({ message: "Password reset is not supported for this role" });
    }
    ...
    if (payload.channel === "email") {
      if (payload.role === "admin") {
        ...
        const otp = String(Math.floor(10000 + Math.random() * 90000));
        const mailResult = await sendOtpEmail({ email: admin.email, otp, role: "admin" });
        ...
        return res.json({
          ...
          message: "Password reset OTP sent again to your email.",
        });
      }
    ...
    const phoneResult = await sendPhoneOtp({ phoneNumber: payload.mobile });
    if (!phoneResult.sent) {
      return res.status(500).json({ message: phoneResult.reason });
    }
    ...
    return res.json({
      resetToken: createPendingPasswordResetToken({
        ...
        userId: payload.userId,
        channel: "phone",
        mobile: phoneResult.phoneNumber || payload.mobile,
      }),
      message: "Password reset OTP sent again to your mobile number.",
    });
  }),
);
  ...
  asyncHandler(async (req, res) => {
    ...
    if (payload.channel === "email") {
      if (payload.role === "admin") {
        ...
        return res.json({
          passwordResetToken: createPasswordResetVerifiedToken({
            role: payload.role,
            userId: payload.userId,
          }),
      ...
      const [user] = await query(
        `SELECT ${config.id} AS id, otp FROM ${config.table} WHERE ${config.id} = ?`,
        [payload.userId],
      );
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    let payload;
    try {
      payload = jwt.verify(passwordResetToken, process.env.JWT_SECRET || "change-this-secret");
    } catch {
      return res.status(401).json({ message: "Password reset session expired. Start again." });
    }
    ...
    if (payload.role === "admin") {
      await query("UPDATE admin SET admin_password = ? WHERE admin_id = ?", [hashedPassword, payload.userId]);
    } else {
      await query(`UPDATE ${config.table} SET password = ?, otp = 0 WHERE ${config.id} = ?`, [
        hashedPassword,
        payload.userId,
      ]);
    }
